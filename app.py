...
def build_summary_text(
    ...
    # group shop stats
    shop_groups = df_filtered.groupby("shopid", dropna=False)
    ...
    lines.append(f"Jumlah Toko (ShopID): {shop_groups.ngroups if total_filtered else 0}")
    lines.append("")
...
def parse_tsv_text(raw_text: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    Return: (df_ok, df_err)
    - df_ok: parsed rows
    - df_err: rows that fail parsing/format
    """
    if not raw_text:
    ...
    for i, ln in enumerate(lines[1:], start=2):  # line number in file (1-based)
        try:
            ...
            # keep original line order as "no"
            row["no"] = i - 1  # similar spirit to JS (row.no = i)
            ok_rows.append(row)
...
def make_csv_bytes(df: pd.DataFrame) -> bytes:
    ...
    for out_col, src_col in cols_map[1:]:
        if src_col is None:
            continue
        out[out_col] = df[src_col] if src_col in df.columns else ""

    # Use semicolon like JS
    bio = io.StringIO()
    out.to_csv(bio, index=False, sep=";")
...
# UI
# =========================
st.title("📌 Data Filter Bot — Streamlit (Auto ShopID, Filter, Export)")

with st.expander("ℹ️ Format input yang didukung", expanded=True):
    st.write(
        "Format file: **TSV (tab-separated)** seperti export kamu:\n\n"
        "`no[tab]link[tab]nama[tab]harga[tab]stok[tab]terjual bln[tab]terjual semua[tab]komisi[tab]komisi rp[tab]rating`\n\n"
        "Kolom angka akan diparse seperti di JS (titik ribuan dihapus, koma jadi desimal)."
    )
...
with colA:
    ...
    raw_text = st.text_area(
        "Atau paste di sini (TSV / tab-separated):",
        value=raw_text,
        height=220,
...
with colB:
    ...
    terjual_min = st.number_input("Minimum Terjual per Bulan", min_value=0.0, value=0.0, step=1.0)
    komisi_min = st.number_input("Minimum Komisi (%)", min_value=0.0, value=0.0, step=0.5)
    komisirp_min = st.number_input("Minimum Komisi (Rp)", min_value=0.0, value=0.0, step=100.0)

    run_btn = st.button("🚀 Jalankan Filter", type="primary")
...
if run_btn:
    ...
    if df_all.empty:
        st.error("❌ Data kosong / format salah. Pastikan ada header + minimal 1 baris data.")
        if not df_err.empty:
            st.dataframe(df_err, use_container_width=True)
        st.stop()

    # Ensure numeric columns exist
    for c in ["terjual bln", "komisi", "komisi rp", "rating", "harga", "stok", "terjual semua"]:
    ...
    if not df_err.empty:
        st.warning(f"⚠️ Ada {len(df_err)} baris error (di-skip). Lihat detail di bawah.")
        st.dataframe(df_err, use_container_width=True)
    ...
    df_filtered = df_filtered.sort_values(["shopid", "no"], ascending=[True, True]).reset_index(drop=True)
    ...
    if total_filtered == 0:
        st.error("Tidak ada data yang memenuhi filter. Coba turunkan nilai minimum.")
        st.stop()
    ...
    st.dataframe(df_filtered[preview_cols].head(25), use_container_width=True)
    ...
    st.info("Upload/paste data, atur filter, lalu klik **Jalankan Filter**.")
